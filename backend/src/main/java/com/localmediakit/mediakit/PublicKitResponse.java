...
public record PublicKitResponse(
        ...
        /** The showcase. Empty on pages published before the section existed. */
        List<MediaKitSnapshot.MediaSnapshot> media,
        boolean showBadge,
    ...
    public static PublicKitResponse locked(MediaKitSnapshot snapshot, MediaKitVersion version) {
        return new PublicKitResponse(
                snapshot.slug(),
                snapshot.title(),
                null, null, snapshot.theme(),
                snapshot.accentOrDefault(), snapshot.layoutOrDefault(),
                snapshot.languageOrDefault(), null,
